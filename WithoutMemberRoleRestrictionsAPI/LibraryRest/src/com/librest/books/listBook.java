...
public class listBook {
	...
	//----------------------------------------------------------------------MEMBER ONLY METHODS -----------------------//  
	  ...
	  @Produces("application/json")
	  @Path("/removeBook")
	  public Response removeBook(@FormParam("ID") String id 
	                     ) {
		  int rs=-1;
		  int statusCode=400;
	  ...
	  @POST
		...
		public Response newBook(
		  ...
		  int rs=-1;
		  int statusCode=400;
		  if(title.equals("") || author.equals("") || coverName.equals("") || pages<0)  return listBook.makeCORS(Response.status(statusCode),"");
		...
		public Response updateBook(
		  ...
		  @Produces("application/json")
		  @Path("/setAvailability")
		  public Response setAvailability(@FormParam("ID") int ID,
				  @FormParam("numChange") int numChange){
	  		int statusCode=400;
	  		if(ID<1 || numChange<1) return listBook.makeCORS(Response.status(statusCode), "");
			  DBConnection dbInstance= new DBConnection();
			  JSONObject jsonObject= new JSONObject();
			  String result="";
			  HashMap<String,String> updateValues = new HashMap<String,String>();
		  ...
		  @Produces("application/json")
		  @Path("/takeBook")
		  public Response takeBook(@HeaderParam("Authorization") String token,
				  @FormParam("ID") int ID,
	 ...
	 //Creates Responses for CORS problems regarding requests
